...
class _ProfilePhotosScreenState extends ConsumerState<ProfilePhotosScreen> {
  ...
  Future<void> _pickAndUpload() async {
    if (_photos.length >= 6) {
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Μέγιστος αριθμός φωτογραφιών: 6')));
      return;
    }
    ...
    try {
      ...
      final storageRef = FirebaseStorage.instance
          .ref('users/$uid/${DateTime.now().millisecondsSinceEpoch}.jpg');
      await storageRef.putFile(File(file.path));
      final url = await storageRef.getDownloadURL();

      final newPhotos   = [..._photos, url];
      final newProfile  = _profilePhoto ?? url;
      setState(() { _photos = newPhotos; _profilePhoto = newProfile; });
      await ref.read(userRepoProvider).update(uid, {
        'photos': newPhotos, 'avatarUrl': newProfile,
      });
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _setAsProfile(String url) async {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Profile photo ενημερώθηκε!')));
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return userAsync.when(
      ...
      error: (_, __) => const Scaffold(
          body: Center(child: Text('Σφάλμα',
              style: TextStyle(color: AppColors.textSecondary)))),
      data: (user) {
        if (user == null) return const Scaffold();
        if (!_initialized) {
          _photos       = List<String>.from((user.toFirestore()['photos'] ?? []) as List);
            ...
            child: Column(children: [
              ...
              Container(
                ...
                child: const Row(children: [
                  ...
                  SizedBox(width: 8),
                  Expanded(child: Text(
                    'Μπορείς να ανεβάσεις έως 6 φωτογραφίες. '
                    'Πάτα μια φωτογραφία για να την ορίσεις ως profile photo.',
              ...
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                Text('${_photos.length}/6 φωτογραφίες',
                ...
                child: _photos.isEmpty
                    ? Center(child: Column(
                        ...
                        ElevatedButton.icon(
                          ...
                          label: const Text('Πρόσθεσε φωτογραφία'),
                        ),
                      ]))
                    : GridView.builder(
                        ...
                        itemBuilder: (_, i) {
                          ...
                          return GestureDetector(
                            ...
                            child: Stack(children: [
                              ...
                              if (isProfile) Positioned(top: 4, left: 4,
                                child: Container(
                                  padding: const EdgeInsets.symmetric(
