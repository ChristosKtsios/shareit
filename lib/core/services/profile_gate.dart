...
///
/// **2. Κινητό — ΠΡΟΑΙΡΕΤΙΚΟ.** Η επαλήθευση κινητού ΔΕΝ μπλοκάρει την είσοδο.
/// Ήταν υποχρεωτική για όλους, αλλά αυτό σήμαινε ότι ένα SMS που δεν έφτανε =
/// χαμένος χρήστης, χωρίς καμία εναλλακτική. Όποιος δεν έχει επαληθευμένο
/// κινητό εμφανίζεται ως **«Μη επαληθευμένος χρήστης»** στο προφίλ του
/// (profile_screen), και μπορεί να επαληθεύσει όποτε θέλει από τις Ρυθμίσεις.
class ProfileGate {
  ProfileGate._();
  ...
  static Future<ProfileGateStep> nextStep() async {
    ...
    try {
      ...
      // ── 2) Κινητό: ΔΕΝ μπλοκάρει ──
      // Ο χρήστης χωρίς επαληθευμένο κινητό μπαίνει κανονικά, αλλά εμφανίζεται
      // ως «Μη επαληθευμένος» στο προφίλ του και μπορεί να επαληθεύσει από τις
      // Ρυθμίσεις.
